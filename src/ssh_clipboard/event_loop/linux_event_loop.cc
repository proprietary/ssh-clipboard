...
namespace ssh_clipboard::event_loop {
  ...
  LinuxEventLoop::LinuxEventLoop(int max_events) : epoll_fd_{-1}, running_{false}, events_{static_cast<size_t>(max_events)} {
    ...
    if (epoll_fd < 0) {
      throw std::system_error{errno, std::generic_category(), "epoll_create1: Failed to create event loop"};
    }
  ...
  void run() {
    while (running_) {
      ...
      if (n < 0) {
        throw std::system_error{errno, std::generic_category(), "epoll_wait: Failed to wait for events"};
      }
      for (int i = 0; i < n; ++i) {
  ...
  void add_fd(int fd, std::function<void(int)> callback) {
    ...
    if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, fd, &event) < 0) {
      throw std::system_error{errno, std::generic_category(), "epoll_ctl: Failed to add file descriptor"};
    }
  ...
  void remove_fd(int fd, Callback callback) {
    if (epoll_ctl(epoll_fd_, EPOLL_CTL_DEL, fd, nullptr) < 0) {
      throw std::system_error{errno, std::generic_category(), "epoll_ctl: Failed to remove file descriptor"};
    }
    callbacks_.erase(fd);
  }
}  // namespace ssh_clipboard::event_loop
