...
namespace ssh_clipboard::event_loop {
  ...
  MacEventLoop::MacEventLoop(int max_events) : poll_fd_{kqueue()}, running_{false}, events_{static_cast<size_t>(max_events)} {
    if (poll_fd_ < 0) {
      throw std::system_error(errno, std::generic_category(), "Failed to create kqueue event loop");
    }
  ...
  void MacEventLoop::run() {
    while (running_) {
      const int n = kevent(poll_fd_, nullptr, 0, events_.data(), events_.size(), nullptr);
      if (n < 0) {
        throw std::system_error{errno, std::generic_category(), "Failed to poll for events"};
      }
      for (int i = 0; i < n; ++i) {
  ...
  void MacEventLoop::add_fd(int fd, std::function<void(int)> callback) {
    ...
    if (kevent(poll_fd_, &event, 1, nullptr, 0, nullptr) < 0) {
      throw std::system_error{errno, std::generic_category(), "Failed to add fd to kqueue"};
    }
  ...
  void MacEventLoop::remove_fd(int fd) {
    ...
    if (kevent(poll_fd_, &event, 1, nullptr, 0, nullptr) < 0) {
      throw std::system_error{errno, std::generic_category(), "Failed to remove fd from kqueue"};
    }
    callbacks_.erase(fd);
  }

  MacEventLoop::~MacEventLoop() {
    close(poll_fd_);
  }
} // namespace ssh_clipboard::event_loop
